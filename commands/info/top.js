        ...
        const generateEmbeds = async () => {
            ...
            for (let i = 0; i < slicedleaderboard.length < 11 ? slicedleaderboard.length : 10; i++) {
                ...
                for (let l = 0; l < page.length; l++) {
                    ...
                    userdataarr.push({ top: l + 1 + (i * 10), avatar: mem.user.displayAvatarURL(), tag: mem.user.username, score: `${memd.level?.lvl ? memd.level.lvl : 1} (${memd.level?.xp ? memd.level.xp : 0})` });
                }
